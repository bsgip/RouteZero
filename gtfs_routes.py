...
def preprocess_selected_route_data(selected_routes, routes_df, shapes_df, stops_df, stop_times_df, trips_df, cutoffs):
    subset_routes = routes_df.loc[routes_df['route_short_name'].isin(selected_routes)]
    subset_trips = trips_df.loc[trips_df.route_id.isin(subset_routes['route_id'])]
    subset_stop_times = stop_times_df.loc[stop_times_df.trip_id.isin(subset_trips['trip_id'])]
    subset_stops = stops_df.loc[stops_df.stop_id.isin(subset_stop_times['stop_id'])]
    subset_shapes = shapes_df.loc[shapes_df.shape_id.isin(subset_trips['shape_id'])]
    ...
    if cutoffs is not None:
        max_stop_time = subset_stop_times['departure_time'].max()/3600
        if cutoffs[-1] < max_stop_time:       # do something about this
            print('Warning: last cutoff ({}) is less than last stop time ({})'.format(cutoffs[-1],subset_stop_times['departure_time'].max()/3600))
